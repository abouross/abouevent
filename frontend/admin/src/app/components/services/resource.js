/**
 * Created by abou on 10/06/17.
 */
(function () {
    'use strict';

    angular.module('AbouEventAdmin')
        .factory('$abresource', AbResource)
        .factory('$abmodel', AbModel);


    function AbResource(model) {
        this.cache = {};
        var self = this;
        return function (name) {
            if (!angular.isString(name))
                throw 'Resource name must be an string';
            ...
            if (!self.cache[name])
                self.cache[name] = model(name);
            return self.cache[name];
        }
    }

    AbResource.$inject = ['$abmodel'];


    function AbModel($http, ApiBaseUrl, $mdToast, $q) {
        return function (name) {
            ...
            BatchDeleter.prototype = {
                ...
                deleteItem: function deleteItem(id) {
                    var deleter = this;
                    var event = $http.delete(url + '/' + id)
                        .then(function (response) {
                            deleter.handleDelete(id);
                            deleter = event = response = null;
                        }, function (response) {
            ...
            return {
                ...
                count: function (query) {
                    return $http.get(url + '/count', {'params': query})
                        .catch(function (err) {
                            $mdToast.show(
                                $mdToast.danger()
                                    .content(err.statusText || 'Erreur de connexion avec le serveur!')
    ...
    AbModel.$inject = ['$http', 'ApiBaseUrl', '$mdToast', '$q'];
})();
