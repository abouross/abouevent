...
trait Validation
{
    ...
    public static function bootValidation()
    {
        ...
        static::extend(function ($model) {
            if ($model instanceof Model)
                $model->bindEvent('saving', function ($data, $options) use ($model) {
    ...
    {
        $validator = app(Factory::class)->make($data, $rules, $customMessages, $attributeNames);
    ...
    public function validate($rules = null, $customMessages = null, $attributeNames = null)
    {
        ...
        if (!empty($rules)) {
            ...
            foreach ($this->casts as $cast => $format) {
                $data[$cast] = $this->getAttribute($cast);
            }
    ...
    protected function hasRelation($attribute)
    {
        if (!(array_key_exists($attribute, $this->attributes) ||
                $this->hasGetMutator($attribute)) && !method_exists(static::class, $attribute)
        )
            return true;
        return false;
    }
