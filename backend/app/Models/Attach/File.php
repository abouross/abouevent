...
class File extends Model
{
    ...
    public function fromPost($uploadedFile)
    {
        ...
        $this->title = ucfirst(substr($this->file_name, 0, mb_strpos($this->file_name, '.')));
    ...
    public function fromFile($filePath)
    {
        ...
        $this->file_name = (method_exists($file, 'getClientOriginalName')) ? $file->getClientOriginalName() : $file->getFilename();
    ...
    public function output($disposition = 'inline')
    {
        ...
        header('Content-Disposition: ' . $disposition . '; filename="' . $this->title || $this->file_name . '"');
    ...
    public function response($disposition = 'inline')
    {
        ...
        $header = [
            ...
            'Content-Disposition' => $disposition . '; filename="' . $this->title || $this->file_name . '"',
            'Cache-Control' => 'private',
            'Accept-Ranges' => 'bytes',
            'Content-Length' => $this->file_size
        ];

        return response($this->getContents(), 200, $header);
    }

    public function responseThumb($width, $height, $options = [])
    {
    ...
    {
//        return $this->getPublicPath() . $this->getPartitionDirectory() . $this->disk_name;7
        return '/file/' . $this->{$this->getKeyName()};
    }
